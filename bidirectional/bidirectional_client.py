...
def grpc_server_on() -> bool:
        TIMEOUT_SEC = 1
        try:
            grpc.channel_ready_future(grpc.insecure_channel('localhost:50051')).result(timeout=TIMEOUT_SEC)
            return True
        except grpc.FutureTimeoutError:
            return False

def apology():
        print("Sorry, server isn't available")
        return

if __name__ == '__main__':
    if grpc_server_on() is True:     run()
    else: apology()
